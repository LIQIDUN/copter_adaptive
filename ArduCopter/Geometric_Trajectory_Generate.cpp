                                               ...
                                               Vector2f *targetYaw_ddot)
{
    ...
    else if (timeInThisRun >= time_takeoff + acc_time && timeInThisRun < time_takeoff + acc_time + T_circle * circle_num)
    { // 圆周
        *in_flight = 1;
        *in_trj = 1;
        float w_circle = 2 * M_PI / T_circle;
    ...
    else if (timeInThisRun >= time_takeoff + acc_time + T_circle * circle_num && timeInThisRun < time_takeoff + acc_time + T_circle * circle_num + acc_time)
    { // 水平减速
        *in_flight = 1;
        *in_trj = 0;
        float time_in_acc = timeInThisRun - (time_takeoff + acc_time + T_circle * circle_num);
        Trajectory_Generate_EXIT_AUTO(time_in_acc,
                                      v_circle,
                                      targetPos,
                                      targetVel,
    ...
    else if (timeInThisRun >= time_takeoff + acc_time + T_circle * circle_num + acc_time && timeInThisRun < time_takeoff + acc_time + T_circle * circle_num + acc_time + time_land)
    { // 降落
        *in_flight = 0;
        *in_trj = 0;
        float time_in_land = timeInThisRun - (time_takeoff + acc_time + T_circle * circle_num + acc_time);
        Trajectory_Generate_ALT_TO_LAND_AUTO(time_in_land,
                                             targetAlt,
                                             time_takeoff,
                                             targetPos,
    ...
    else if (timeInThisRun >= time_takeoff + acc_time + T_circle * circle_num + acc_time + time_land)
    {
        // 停在地面2*acc_range
        *in_flight = 0;
        *in_trj = 0;

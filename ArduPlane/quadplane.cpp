...
void QuadPlane::update(void)
{
    ...
    if (tiltrotor.dcptilt_transition_active) {
        ...
        // v3.29 FW yaw comes from Plane::stabilize_yaw(), which runs the
        // shared heading target through ArduPlane's native yaw-rate PID.
        // This stage performs only the controller-allocation operation:
        //     FW yaw actuator command = FWW * AP-native raw rudder.
        const float rudder_raw =
            SRV_Channels::get_output_scaled(SRV_Channel::k_rudder);
...
void QuadPlane::motors_output(bool run_rate_controller)
{
    ...
    if (run_rate_controller) {
        ...
        // DCPTilt experimental multicopter-controller weighting. Apply the
        // selected 3x6 MCW allocation consistently on all multicopter axes so
        // yaw authority is transferred to the fixed-wing rudder in the same
        // complementary way as roll/pitch are transferred to the aerodynamic
        // surfaces. This keeps the transition logic aligned with the intended
        // controller-allocation design: MCW + FWW = 1.
        if (tiltrotor.dcptilt_transition_active) {
